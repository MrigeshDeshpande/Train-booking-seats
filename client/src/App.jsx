...
<div className="controls">
  <div className="recently-booked-display">
    <span><strong>Book Seats</strong></span>
    <div className="selected-seat-list">
      {recentlyBooked.map((num) => (
        <div key={num} className="selected-seat">{num}</div>
      ))}
    </div>
    <input
      type="number"
    ...
    <div className="button-row">
      ...
      <div className="legend-bar">
        ...
        <div className="legend-item available">Available Seats = {availableCount}</div>
        {recentlyBooked.length > 0 && (
          <div className="legend-item recently-booked">Just Booked = {recentlyBooked.length}</div>
        )}
      </div>
    </div>
  )
